:-module(minmax_better, [minimax_better/5]).
:-use_module('win.pl').
...
best(D,B,M,[S1],S,U) :-
    ...
    !,  
    minimax_better(D,B2,M2,_S,U),  % then recursively search for the utility value of that move.
    S = S1, !,
    output_value(D,S,U),
    !
    .
...
best(D,B,M,[S1|T],S,U) :-
    ...
    inverse_mark(M,M2), 
    !,
    minimax_better(D,B2,M2,_S,U1),  % recursively search for the utility value of that move
